...
import static com.Menu.SQLite_2.conn;
import java.io.File;
...
//    static File temp = new File("db.sqlite");
    //static String url = "jdbc:sqlite:"+temp.getAbsolutePath().replace("\\","\\\\");
    static String url = "jdbc:sqlite:C:\\Users\\Acer\\Desktop\\waka_massage_system\\WakaMassagePalaceSystem\\src\\com\\Menu\\db.sqlite";
    static String error = ""; 
...
public static boolean updateItem(String table, String set, int itemNo){
        ...
        try{
            SQLite.stmt = conn.createStatement();
            String query = "UPDATE "+ table +" SET " + set + " WHERE itemNo=" + itemNo;
            stmt.executeUpdate(query);
            //You can include exception handling here. (e.g. Duplicate Data, etc.)
            result = true;
        }
...
public static String[][] readOnly(String table){
        ...
        try{
            ...
            //Count total rows
            ResultSet rs = stmt.executeQuery("SELECT ID, firstName, lastName, gender FROM " + table);
            int totalRows = rs.getInt(1);
